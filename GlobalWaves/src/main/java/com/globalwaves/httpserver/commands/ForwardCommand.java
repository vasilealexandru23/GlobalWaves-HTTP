...
public final class ForwardCommand extends Command implements CommandRunner {
    public ForwardCommand(final String command, final String username,
    ...
    public ObjectNode execute(final Database database) {
        ...
        if (!user.isOnline()) {
            ...
            output.put("message", forwardTrack(user.getMusicPlayer()));
        }

        return output;
    }

    private String forwardTrack(final MusicPlayer player) {
        ...
        return ((Podcast) player.getPlayback().getCurrTrack()).forward(player.getPlayback());
    }
}
